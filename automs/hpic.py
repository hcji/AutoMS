...
def PIC(inputfile, min_intensity=200, mass_inv=1, rt_inv=15, mis_gap=3, max_items=30000):
    ...
    for k, pic in pic_list.items():
        if len(pic) >= 2:
            pic_list_refine[k] = pic
    
    return rt_mean_interval, pic_list_refine
...
def to_deque(pic_list, min_snr, rt_v, intensity):
    ...
    peak_list = []
    alls = []
    files = np.array(list(pic_list.keys()))
    for each in files:
        each = [float(s) for s in each.split('_')]
        ...
        if i not in count:
            ...
            if length >1:
                ...
                if condition[(condition>0)&(condition<rt_v)].shape[0] >0 :
                    ...
                    mass = np.vstack([pic_list[file_i] for file_i in list(files[index])])              
                    new[3] = mass[0,0]
                    new[4] = mass[-1,0]
                    new[5] = mass.shape[0]
                    if new[5] >=number:
                        label = '%s_%s_%s_%s_%s_%s' % (new[0],new[1],new[2],new[3],new[4],new[5])
                        new_pic_list[label] = mass
                        peaks = lc_ms_peak(mass,width,min_snr,True,intensity)
                        peaks = [np.append(p, label) for p in peaks]
                        peak_list.extend(peaks)                       
                    count.extend(index)
                else:
                    if frame[i,5]>=number:
                        label = files[i]
                        mass = pic_list[files[i]]
                        new_pic_list[label] = mass
                        peaks = lc_ms_peak(mass,width,min_snr,True,intensity)
                        peaks = [np.append(p, label) for p in peaks]
                        peak_list.extend(peaks)
    ...
    peak_list = pd.DataFrame(peak_list, columns = ['mz','rt','intensity','rt1','rt2','signal','snr','ind','shape','pic_label'])
    for col in peak_list.columns[:-1]:
        peak_list[col] = peak_list[col].astype(float)
...
if __name__ == '__main__':
    ...
    peak_list, pic_list = hpic(file_in, min_intensity=2000, min_snr=3, mass_inv=1, rt_inv=15)
